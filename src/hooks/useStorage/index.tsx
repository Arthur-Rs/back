...
const StorageProvider: FC = ({ children }) => {
  ...
  const saveGoal = useCallback(
    async (data: CreateGoalDTO): Promise<void> => {
  ...
  const getOneGoal = useCallback(async (id: string): Promise<
    GoalEntity | undefined
  > => {
    const strGoals = await AsyncStorage.getItem('@jotting:goals')
    if (!strGoals) {
      return undefined
  ...
  const saveFinance = useCallback(
    async (data: CreateFinanceDTO): Promise<void> => {
  ...
  const getOneFinance = useCallback(async (id: string): Promise<
